...
discovered from the verified catalog-instantiation/PMDL closure.  The generic PMDL
...
from ..physics.backend import Backend, infer_backend
...
from ..physics.resolved import DynamicsCompletenessRecord, ResolvedAssembly, resolve_assembly
...
def _require_scanner_dynamics_completeness(
    ...
    resolved_part = assembly.parts[chassis.part]
    try:
        body = resolved_part.body_map[str(estimate["part_body"])]
    except KeyError as exc:
        raise ScannerRuntimeError(
            f"scanner yaw inertia references missing part body {exc.args[0]!r}"
        ) from exc
    solids = [solid for solid in body.solids if solid.id == estimate["part_solid"]]
    solid = _one(solids, "scanner yaw inertia part solid")
...
def _derive_layout(assembly: ResolvedAssembly) -> _ScannerLayout:
    ...
    def wheel_details(wheel: str) -> tuple[str, str]:
        ...
        wheel_part = assembly.parts[
            next(item.part for item in assembly.specification.components if item.id == wheel)
        ]
        axle_connectors = [
            connector for connector in wheel_part.connectors if connector.model_port == "axle"
        ]
    ...
    camera_part = assembly.parts[
        next(item.part for item in assembly.specification.components if item.id == camera)
    ]
    optical_connector = _one(
        [
            connector
            for connector in camera_part.connectors
    ...
    **simulator_options: Any,
    ...
    if not isinstance(assembly, ResolvedAssembly):
        raise TypeError(
            ...
            "contraption/parts/PMDL closure first"
        )
    _require_scanner_dynamics_completeness(assembly)
    canonical_mission = ScannerMission.from_assembly(assembly)
