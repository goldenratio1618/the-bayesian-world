"""Isolated, deterministic validation tool for modeling-agent PMDL drafts.

The tool deliberately has one narrow capability: validate a ``.pmdl`` file
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Validate one PMDL draft inside an isolated modeling workspace.",
    )
    parser.add_argument("candidate", help="relative candidate/**/*.pmdl path")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = validate_candidate(args.candidate)
