...
model equations remain strings until :mod:`contraption.dsl` parses them into
...
class ModelSpec(StrictRecord):
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ModelSpec":
        ...
        names = (
            "format", "id", "name", "version", "domains", "category", "description", "power_ports", "signal_ports",
        ...
        return cls(
            ...
            _identifier(data["category"], "model.category"), _string(data.get("description", ""), "model.description"),
...
class GeometrySpec(StrictRecord):
    ...
    def __post_init__(self) -> None:
        if len(self.dimensions) != 3 or any(value <= 0 or not math.isfinite(value) for value in self.dimensions):
            raise SpecError("geometry.dimensions must contain three finite positive values")
        if self.kind not in {"box", "mesh", "cylinder", "sphere", "composite"}:
            raise SpecError(f"unsupported geometry kind {self.kind!r}")
        if self.kind == "mesh" and not self.uri:
            raise SpecError("mesh geometry requires a uri")
        object.__setattr__(self, "metadata", _freeze(self.metadata, "geometry.metadata"))
...
class ComponentInstanceSpec(StrictRecord):
    ...
    def __post_init__(self) -> None:
        ...
        _identifier(self.model, "component.model")
        if self.condition not in {"unverified", "inspected", "calibrated", "degraded", "retired"}:
            raise SpecError(f"unsupported component condition {self.condition!r}")
        object.__setattr__(self, "parameters", _freeze(self.parameters, "component.parameters"))
        object.__setattr__(self, "metadata", _freeze(self.metadata, "component.metadata"))
...
SignalBindingSpec = ControlBindingSpec
...
class ContraptionSpec(StrictRecord):
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ContraptionSpec":
        ...
        return cls(_string(data["format"], "contraption.format"), _identifier(data["id"], "contraption.id"),
                   ...
                   tuple(ComponentInstanceSpec.from_dict(item) for item in _sequence(data["components"], "contraption.components")),
...
def json_schema_for(record_type: type[StrictRecord]) -> dict[str, Any]:
    ...
    def visit(cls: type[StrictRecord]) -> None:
        ...
        constants = {
            ...
            ("ContraptionSpec", "format"): "contraption-1",
            ("Taxonomy", "format"): "taxonomy-1",
        }
