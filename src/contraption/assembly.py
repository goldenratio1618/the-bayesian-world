...
protocol consumed by :func:`contraption.simulator.simulate`, so the same object
...
def _component_input(record: Any) -> _ComponentInput:
    ...
    if model_reference is None:
        model_reference = _record_field(record, "model")
...
def assemble_contraption(
    specification: ContraptionSpec | Mapping[str, Any],
    ...
    *,
    ...
    ``component_models`` resolves PMDL behavior by component-instance ID and
    therefore avoids duplicating a package-owned model ID in the contraption.
    Values may be exact :class:`ModelSpec` objects or IDs in ``model_registry``.
    When it is absent, a resolved component's ``model_id`` is used, with
    legacy ``model`` accepted for contraption-1.

    ``connector_bindings`` is the temporary bridge between a typed physical
    connector package and PMDL behavior ports.  Missing bindings are never
    ...
    produced by the package/geometry resolver.  It becomes the shared artifact
    ...
    if isinstance(specification, ContraptionSpec):
        ...
        spec = ContraptionSpec.from_dict(specification)
    elif all(
        hasattr(specification, name)
        for name in ("components", "connections", "controls", "to_dict")
    ):
        # A future package-resolved contraption record can use the same narrow
        # protocol without being coerced back through the legacy schema.
        spec = specification
    else:
        raise TypeError(
            "specification must be a ContraptionSpec, source mapping, or resolved "
            "contraption record with components/connections/controls/to_dict"
        )
