...
class ModelRegistry(Mapping[str, ModelSpec]):
    ...
    def register(self, model: ModelSpec, *, validate: bool = True) -> ModelSpec:
        if model.id in self._models:
            raise SpecError(f"model id {model.id!r} is already registered")
        if validate:
            from .validation import validate_model
            validate_model(model).require_valid()
        self._models[model.id] = model
        return model

    def load(self, path: str | Path, *, validate: bool = True) -> ModelSpec:
        return self.register(load_model(path), validate=validate)

    def load_directory(self, directory: str | Path, *, recursive: bool = True, validate: bool = True) -> tuple[ModelSpec, ...]:
        root = Path(directory)
        paths = sorted(root.rglob("*.pmdl") if recursive else root.glob("*.pmdl"))
        return tuple(self.load(path, validate=validate) for path in paths)
